...
public class crocController : MonoBehaviour {
	...
	void Update () {
        ...
        {
            transform.position = Vector2.MoveTowards(transform.position, endSpot, speed * Time.deltaTime);
            Vector2 direction = endSpot - transform.position;
            float angle = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg + 90;
            Quaternion rotation = Quaternion.AngleAxis(angle, Vector3.forward);
            transform.rotation = Quaternion.Slerp(transform.rotation, rotation, 3f * Time.deltaTime);
            transform.position = Vector2.MoveTowards(transform.position, endSpot, speed * 1.5f * Time.deltaTime);
            if (this.transform.position == endSpot)
            {
                Destroy(this.gameObject);
            }
